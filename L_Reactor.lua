...
local _PLUGIN_VERSION = "3.9develop-20291"
...
local function plugin_runOnce( pdev )
	...
	initVar( "NetworkStatus", "", pdev, MYSID )
...
local function updateNetworkStatusHistory( flag, pdev )
	D("updateNetworkStateHistory(%1)", flag)
	local nstates = getReactorVar( "ns", "0:X" , pdev, MYSID )
...
--]]
	...
	-- Update Internet check configuration
	if not isOpenLuup then
		local ci = getVarNumeric( "InternetCheckInterval", 5, pdev, MYSID )
		if ci > 0 then
			D("startPlugin() writing reactor_internet_check.conf")
			local sl = getReactorVar( "InternetCheckSites", "", pdev )
			local fd = io.open( "/etc/cmh-ludl/reactor_internet_check.conf", "w" )
			...
			if sl ~= "" then
				L("Internet check servers: %1", sl)
				fd:write('SERVERS="'..sl..'"\n')
			end
			fd:close()
			if file_exists( "/etc/cmh-ludl/reactor_internet_check.sh.lzo" ) and 
				not file_exists( "/etc/cmh-ludl/reactor_internet_check.sh" ) then
			...
			if not file_exists( "/etc/init.d/reactor_internet_check" ) then
				L("Creating Reactor's Internet check daemon...")
				fd = io.open( "/etc/init.d/reactor_internet_check", "w" )
				fd:write( [[#!/bin/sh /etc/rc.common
...
start_service () {
	...
	procd_close_instance
				...
				fd:close()
				os.execute( "chmod +rx /etc/init.d/reactor_internet_check" )
				os.execute( "rm -f /etc/rc.d/S*reactor_internet_check /etc/rc.d/K*reactor_internet_check" )
				os.execute( "cd /etc/rc.d/ && ln -sf ../init.d/reactor_internet_check S85reactor_internet_check" )
				os.execute( "/etc/init.d/reactor_internet_check stop" )
				os.execute( "/etc/init.d/reactor_internet_check start" )
			end
		else
			-- Internet check is off.
			setVar( MYSID, "NetworkStatus", "", pdev )
			W("Internet status checks disabled.")
			os.execute("/etc/init.d/reactor_internet_check stop")
			os.remove("/etc/init.d/reactor_internet_check")
			os.remove("/etc/cmh-ludl/reactor_internet_check.conf")
		end
	end
...
local function sensorWatch( dev, sid, var, oldVal, newVal, tdev, pdev )
	...
	elseif sid == RSSID and var == "TestTime" then
		if newVal == "" then
			...
			local tr = os.time()
			local tt = tonumber(newVal) or tr
			if ( tt % 60 ) == 0 then tr = math.floor( tr / 60 ) * 60 end
			setVar( RSSID, "tref", tr, dev )
		end
	else
		addEvent{ dev=tdev, event='devicewatch', device=dev,
