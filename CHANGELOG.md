...
## Version 3.12 (22321, development)

* Reactor activities now run in the background and asynchronously by default. This assists with error resistance in overall ReactorSensor state.
* Additional error checking when running activities, to log more info when something doesn't go right, particularly in a Vera scene being executed in an activity (in line)
* Add UIs for ignoring server certificates and following redirects on *HTTP Request* actions, as these are commonly used.
* Initialize more state variables for various options so they are easier to find/don't need to be created when needed.
