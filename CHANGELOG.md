...
* Enhancement: Tighten integration with VeraAlerts for users who use it; Reactor will attempt to import (automatically) overrides of the notification message and recipients done in VA.
* Enhancement: The *Notify* action will send a notification to the selected users with a custom message. The message cannot be dynamic (e.g. the use of variable references in the message field like `{message}` or `The current temperature of {temp} is over limit!`) are not possible currently--sorry, we're working within the limits of Vera here; the alternative is one of the third-party notifications plugins that you can drive with Device Actions). Also, the implementation is heavily dependent on Vera's current mechanisms for sending notifications (specifically, the use of hidden scenes and eventList2), which could change in future firmware.
